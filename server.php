<?php
	...
	switch($cmd){
	   ...
	   <?php
		...
		case "process_bulk_sms":{
			...
			$fromNum  = '31555686708'; //DBin($_REQUEST['from_number']);
			$groupID  = DBin($_REQUEST['group_id']);
			$phoneID  = DBin($_REQUEST['phone_number_id']);
			...
			$deviceID = null; //(int) $_REQUEST['device_id'];
			
			if($client=='all')    /** Envio para todas las cuentas */
			{ 
				if($bulkType=='1'){/** Radio button de numero unico o fechas */
					if($groupID=='all')/* aqui se envia el mensaje a todas las campañas registradas*/
					{
						$sql = sprintf("select 
										c.id, 
										c.title, 
						...
						if(mysqli_num_rows($res)){
							...
							while($row = mysqli_fetch_assoc($res)){
								...
								mysqli_query($link,$ins);
								DBout($index++);
								$messages["number"]=$toNumber; //////////
								$messages["message"]=$smsText;
								$messages["type"]=1;
								$mensa[$suma]=$messages;
								$suma++;
							}
							/****************   Envio de mensajes masivos********************************** */
							$mensajito["name"]="Esta es una prueba";
							$mensajito['sendingDate']="22/04/2021";
							$mensajito['messages']=$mensa;

							require_once './send_sms.php';
							$mensaje= new mensaje();

							$respuesta=$mensaje->enviar_mensaje_masivos(json_encode($mensajito));
							echo $respuesta;
							/******************************************************************* */
					...
					else
					{
						...
						/*Aqui se envia a todos los numeros registrados en una campaña*/
						if($phoneID=='all')
						{
							$sql = sprintf("select 
											s.phone_number 
							...
							if(mysqli_num_rows($res)){
								...
								while($row = mysqli_fetch_assoc($res)){
									...
									$datos["number"]=$tonumber;
									$datos["message"]=$smsText;
									$datos["type"]=0;
									$grupo_mensaje[$count]=$datos;
									$count++;
								}
								/********************************************************* */
								require_once './send_sms.php';
								...
								$message['name']="Mensajes para todos";
								$message['sendingDate']="22/04/2021";
								$message['messages']=$grupo_mensaje;
								$respuesta=$mensaje->enviar_mensaje_masivos($message);
								...
								if($res->{"success"})
								{
									...
									$_SESSION['message_status'] = 0;
									$_SESSION['message'] = 'Failed! No message sent found.';	
								}
						...
						{ /** Aqui se envia el mensaje a un solo numero  */
							echo "Entro aqui 2";
							...
							if(mysqli_num_rows($res)){
								...
								///////////////////// Esto lo hizo miguel ///////////////////////////////////////////								
									...
									$res=$consultas->Dato_aplicacion_settings_id($_SESSION['user_id']);
									if($fila=mysqli_fetch_assoc($res))
									{
									
										if($fila["sms_gateway"]=="cellvoz")
										{
										
											/**Envio de menjaje*/
											require_once 'send_sms.php';
											$mensaje=new mensaje();
											$datos["numero"]=$toNumber;
											$datos["mensaje"]=$smsText."/".$smsMedia;
											$datos["type"]=1;
											$respuesta=$mensaje->enviar_mensaje_solo($datos);
											echo $respuesta;
											$res=json_decode($respuesta);
											if($res->{"success"})
											{
			...
			else /*****************************************Envio para una cuenta en especifico */
			{
				if($bulkType=='1')/***************   LISTO    *************************************** */
				{ 

					if($groupID=='all')/********* Este Ok--   Este es para enviar a todas las camapañas */
					{
						require_once './src/consultas.php';
						$consultas= new consultas();

						$res=$consultas->Telefono_subscriptors_all_campains(DBout($_SESSION['user_id']));
						if(mysqli_num_rows($res)>0){
							$grupo_mensa=array();
							while($row = mysqli_fetch_assoc($res)){
								$datos=array();
							...
							$message['name']="Mensaje__todos_numeros_campaings";
							$message['messages']=$grupo_mensa;
							require_once './send_sms.php';
							$mensaje=new mensaje();
							
							$respuesta=$mensaje->pasarela_validacion_masivo(($message));
							$res=json_decode($respuesta);
							
							if($respuesta==403)
							{
								$_SESSION['message_status'] = 0;
								$_SESSION['message'] = 'Failed! CODE: 403 cellvoz invalid.';
							}
							else if($respuesta==1)
							{
								$_SESSION['message_status'] = 0;
								$_SESSION['message'] = 'Failed! cellvoz found.';
							}
					...
					else
					{
						...
						if($phoneID=='all')/**Este Ok -- Envio de mesajes a todos los subs por camapaña */
						{
							require_once './src/consultas.php';
							$consultas= new consultas();
							$res=$consultas->Telefono_subscriptors_group_campaing(DBin($groupID));
							$grupo_mensa=array();
							...
							if(mysqli_num_rows($res)){
								...
								$message['name']="Mensaje__todos_numeros";
								$message['messages']=$grupo_mensa;
						...
						else /************* Este OK -----enviar mensaje a un solo subscriptor  ******/
						{ 
							require_once './src/consultas.php';
							$consultas=new consultas();
							$res=$consultas->Telefono_subscriptor_id(DBin($phoneID));
							echo "Entro en subscripto";
							if(mysqli_num_rows($res)>0){
								$row = mysqli_fetch_assoc($res);
								$toNumber   = DBout($row['phone_number']);
								$fromNumber = $fromNum;								

								$respu=$consultas->Dato_aplicacion_settings_id($_SESSION['user_id']);
								if($fila=mysqli_fetch_assoc($respu))
								{
									...
									if($fila["sms_gateway"]=="cellvoz")
									{
										...
										$datos["message"]=$smsText;
										$datos["type"]=1;
										$respuesta=$mensaje->pasarela_validacion_solo($datos);
								...
								{
									$_SESSION['message_status'] = 0;
									$_SESSION['message'] = 'Failed! No pasarela selected .';
									echo "No se tiene ninguna pasarela seleccionada";
								}
								/////////////////////// Hasta aqui  ////////////////////								
							}
							else{
					...
					if($_REQUEST['daterange_group_id']=='all') /****** En caso de que sea todas las campañas */
					{
						...
						$exe=$consultas->Telefono_subscriptors_all_campains_date($client, $startDate,$endDate);
					...
					else
					{
						...
						require_once './src/consultas.php';
						$consultas= new consultas(); 
						$exe = $consultas->Telefono_subscriptors_group_campaing_date($groupID, $startDate, $endDate);
						if(mysqli_num_rows($exe)>0){
								$grupo_mensa=array();
								while($numbers = mysqli_fetch_assoc($exe)){
									$toNumber  = $numbers['phone_number'];
							...
							$_SESSION['message_status'] = 1;
							$_SESSION['message'] = 'Success! message sent to all subscribers.';
						}else{
							$_SESSION['message_status'] = 0;
							$_SESSION['message'] = 'Failed! no subscriber found.';
						}
		...
		case "update_sms_gateways":{
			if(trim($_REQUEST['enable_sender_id'])==null)
				$_REQUEST['enable_sender_id'] = '0';

			if(trim($_REQUEST['enable_whatsapp'])==null)
				$_REQUEST['enable_whatsapp'] = '0';
			
			if($_REQUEST['sms_gateway']=="cellvoz" && isset($_REQUEST['Cell_AKey_voz']) && !is_null($_REQUEST['Cell_AKey_voz']))/*** si la paserla de pago fue escogida cellvoz***/ 
			{
				
					$curl = curl_init();
					/** Se pide usuario y contraseña de la pasarela**/ 
					$mandar['account']=$_REQUEST['Cell_user_voz'];
					$mandar['password']=$_REQUEST['Cell_pass_voz'];
					...
					if($response->{"token"}){ /**Si devuelve el token**/
						...
						$user=$_REQUEST['Cell_user_voz'];
						$password=base64_encode($_REQUEST['Cell_pass_voz']);
						$token=base64_encode($response->{'token'});
						$Apikey =base64_encode($_REQUEST['Cell_AKey_voz']);
						
						$pasarela=$_REQUEST['sms_gateway'];
						$id=$_SESSION['user_id'];
						require_once './src/consultas.php';
						$consultas= new consultas();
						...
						$resp=$consultas->Mostrar_sms_cellvoz_id($id);
						if(mysqli_num_rows($resp)>0)
						{
							...
							echo "<script>confirm('Error en la actualización')</script>";
						}
					}
					else{
						$token=base64_encode('NAnA');
						echo "<script>confirm('Datos erroneos')</script>";
						header('Location: settings.php');
	...
	<?php
		...
		case "delete_subscriber":{
			$id = $_REQUEST['id'];
			$up = sprintf("update subscribers set status='3' where id='%s'",
		...
		case "save_scheduler":{
			...
			if(isset($_REQUEST['group_id']) && $_REQUEST['group_id']!="0"){
				...
				if(isset($_REQUEST['phone_number'])){
					...
					if($phone_number=="all")
					{
						...
						$resp=$consultas->Telefonos_subscriptors_campaing($campaing);
						...
						if(mysqli_num_rows($resp)>0)
						{
							...
							{
								$message=array();
								$message["codeCountry"]="57";
								$message["number"]=$fila["phone_number"];
					...
					{
						$phone_subs=$_REQUEST['phone_number'];
						$resp=$consultas->Telefono_subscriptor_id($phone_subs);
			...
			$time = date('Y-m-d',strtotime(DBin($_REQUEST['date'])));
			$time = $time.' '.DBin($_REQUEST['time']);
			if(isset($_REQUEST['send_immediate']))
			{
				$send_immediate = (int) $_REQUEST['send_immediate'];	
			}
			else
			{
				$send_immediate = 0;
			...
			if(isset($_REQUEST['attach_mobile_device']))
			{
				if($_REQUEST['attach_mobile_device'] !='1')
				{
					$attach_mobile_device = '0';	
				}
				else
				{
			...
			require_once './src/consultas.php';
			$consultas= new consultas();
			$res = $consultas->insert_sms_programers_schedulers($_REQUEST['title'], $time, $_REQUEST['group_id'], $_REQUEST['phone_number'], $_REQUEST['message'], $fileName, $_SESSION['user_id'], $attach_mobile_device, $send_immediate, $_REQUEST['custom'], $_REQUEST['search']);
			

			$cuerpo["name"]=$_REQUEST['title'];
			$cuerpo["sendingDate"]=$time;
			$cuerpo['messages']=$messag;

			require_once './send_sms.php';
			$mensaje= new mensaje();
			var_dump(json_encode($cuerpo));
			$respuesta=$mensaje->pasarela_validacion_masivo($cuerpo);
			if($respuesta==403)
			{
				$_SESSION['message_status'] = 0;
				$_SESSION['message'] = 'Failed! CODE: 403 cellvoz invalid.';
		...
		case "get_scheduler_numbers":{
			...
			$exe=$consultas->Telefonos_subscriptors_campaing(DBin($groupID));
			$res=array();
