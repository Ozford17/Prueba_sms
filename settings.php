...
$row_wa=array();
$resp= $consul->users_telem($_SESSION['user_id'],4);
if($fila= mysqli_fetch_assoc($resp)){
	$row_wa= $fila;
}

$whatsapp_business_numbers = $row_wa['phone_number'];
					...
					<div class="card">
						...
						<div class="content table-responsive">
							...
							<div class="tab-content" id="tab_content">
								...
								<div class="tab-pane active" id="sms_gateways">
									<form method="post" action="server.php" enctype="multipart/form-data">
											<?php
												$twilio = 'display-2';
												$plivo  = 'display-2';
											...
											<div class="form-group">
												...
												<select name="sms_gateway" class="form-control smsGateWay">
													...
													<option <?php if($row['sms_gateway']=='signalwire')echo 'selected="selected"';?> value="signalwire">SignalWire (Premium)</option>
													<option <?php if($row['sms_gateway']=='cellvoz')echo 'selected="selected"';?> value="cellvoz">CellVoz </option>
													<option value="teleapi" disabled>Tele API (Coming soon)</option>
												</select>
											</div>
											...
											<div class='CellVoz'>
												<div class='form-group'>
													<Label>CellVoz User</Label>
													<input type="text" class='form-control' name='Cell_user_voz'>
												</div>
												<div class='form-group'>
													<Label> CellVoz Password</Label>
													<input type="text"class='form-control' name='Cell_pass_voz'>
												</div>
												<div class='form-group'>
													<Label> CellVoz Api key</Label>
													<input type="text"class='form-control' name='Cell_AKey_voz'>
												</div>
											</div>	
								...
								<div class="tab-pane " id="buy_numbers">
									<?php
										if($appSettings['sms_gateway']=='twilio')
										{
												if($_SESSION['user_type']=='1'){ 
													?>
													<table width="100%" aling="center" id="top" >
														<tr>
																			...
																			<?php
																				...
																				if(mysqli_num_rows($resStats)){
																					while($rowStats = mysqli_fetch_assoc($resStats)){
																						echo '<option value="'.DBout($rowStats['Code']).'"> '.DBout($rowStats['State']).' ?></option>';
																					}
																				}
																			?>
																		</select>
																			...
																			<select name="state" id="state" class="form-control" onchange="getareacodes(this);" >
																				<?php
																					...
																					if(mysqli_num_rows($resStats))
																					{
																						while($rowStats = mysqli_fetch_assoc($resStats)){
																						echo '
																						   <option value="'. DBout($rowStats['Code']).'">'. DBout($rowStats['State']) .'</option>';
										...
										else if($appSettings['sms_gateway']=='nexmo')
										{
												if($_SESSION['user_type']=='1'){ 
													?>
													<table width="100%" aling="center" id="pattrens1">
														<tr>
															<td colspan="1" width="25%">Select One</td>
															<td id="left" colspan="3">
																<label>Buy Number:&nbsp;
										...
										else if($appSettings['sms_gateway']=='signalwire'){
												if($_SESSION['user_type']=='1')
												{ // admin
													?>
													<div class="form-group">
